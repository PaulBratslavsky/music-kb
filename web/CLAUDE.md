...
Guidance for Claude Code working in `web/`.
...
It's a static SPA — no backend, no database, no accounts.

**This directory is one workspace of the `music-kb` monorepo.** It used to be
its own repo (`music-push-guitar-piano-helper`) and was folded in here so the
two apps stop copying the music-theory layer back and forth. The theory layer
now lives in `packages/music` (`@music-kb/music`) and is shared, not duplicated
— see the root `CLAUDE.md` for the repo shape.
...
- `@music-kb/music` for all music theory (which wraps `tonal`)
- SVG for all instrument views
- Web Audio API for playback (custom 50-line synth in `src/audio/synth.ts`)
- React `useState` + URL params for state — no backend, no DB, no auth

## Working in the codebase

Yarn workspaces, so install once from the repo root (`yarn install`) — never
run a bare `npm install` here, it would fight the root lockfile.
...
it automatically. Verify visually via the dev server; Playwright works against
`localhost`.

## Code structure

Theory, instrument layouts, and the shared domain types are **not here** — they
live in `packages/music` and arrive via `@music-kb/music/...`. What's left in
`web/src` is this app's own views and state:
...
Conventions:
- `packages/music` contains music theory only — no React, no rendering. If a change belongs there, make it there; both apps get it.
...
- `@music-kb/music/theory/positions` has two representations:
...
- **<https://www.guitarscale.org/>** — guitar scale shapes, all 12 keys × all common scales. No API.
  - Page slug: `https://www.guitarscale.org/<root>-<type>.html` — sharps are
    ...
    `@music-kb/music/theory/scales` `guitarScaleOrgUrl()` builds this; the Guitar panel
  ...
  - Source of truth for `@music-kb/music/theory/positions`. Verify by note math (pc of each
...
- Open-position guitar chords: `@music-kb/music/theory/voicings/guitar-shapes` `OPEN_CHORD_SHAPES` (C/A/G/E/D maj, Am/Em/Dm, E7/A7/D7/G7/C7/B7) are voicing index 0 so e.g. C maj defaults to open `x32010`. Barre shapes follow.
...
- **Backlog carried over from the knowledge-base app**, now portable because the
  theory layer is shared: a triads lesson, a power-chords lesson, and minor-key
  chord substitutions in the theory tools.
