...
const DEFAULT_STATE: AppState = {
  ...
  scalePosition: 'all',
  preferFlats: false,
  // This app has no triad/seventh toggle — the knowledge-base app does, and
  // the two share the state type. Fixed at 'triad' rather than made optional
  // so the surface that does offer the choice keeps an exhaustive check.
  chordDepth: 'triad',
};
...
export function stateFromUrl(search: string): AppState {
  ...
  return {
    ...
    scalePosition,
    preferFlats,
    chordDepth: DEFAULT_STATE.chordDepth,
  };
}
