...
- **A CSS variable an exported SVG uses must be in that app's `VAR_NAMES`.** `png-export.ts` rasterizes an SVG detached from the stylesheet, so it inlines the theme — but only the variables on its allowlist. An omission does not degrade, it **inverts**: an unresolved `var()` makes the presentation attribute invalid, so the property takes its INITIAL value. `stroke` becomes `none`, so lines *vanish* rather than turning black, and `fill` becomes black, so dots and labels go solid black — a label drawn in a light token ends up black-on-black and disappears. **It is invisible in the app** (on screen the vars resolve normally) and shows up only in a downloaded file. It shipped that way once: web's list was adapted from the client's and kept the visualizer's token names while `ChordDiagram` draws with the aliases `styles.css` layers on top (`--ink`, `--ink-muted`, `--line`, `--card`), so exported chord diagrams were black dots on blank paper with no fret grid. Both apps now pin it — `web/src/music/png-export.test.ts` and `client/src/lib/music/png-export.test.ts` scan the source of every component that can land inside an exported SVG and fail on any `var()` the list omits. Each keeps its own list and its own `EXPORTABLE_SOURCES`; the two apps' views are separate by [ADR 0009](docs/adr/0009-monorepo-with-shared-music-package.md) and their themes spell the same colours differently.
