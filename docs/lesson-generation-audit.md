...
### 6. Declared but never rendered — four times — **fixed**
...
**Fixed 2026-08-25.** The fourth, `Lesson.instrument`, was still generated by
the planner, stored by `saveLessonService` and shown nowhere — it was found by
the Tier 4 guard below on the guard's first run, which is the point of writing
one. It now renders as a chip on the `/lessons` cards and in the lesson-page
eyebrow, omitted when it is `any` (the default, which carries no information).
...
9. **A render-reachability test.** Done (2026-08-25) —
   `client/src/components/lesson/render-reachability.test.ts`. Both sides are
   derived at test time: the Strapi component + content-type schemas are read
   off disk, and the client render path is parsed with the TypeScript AST for
   real property reads (dot access, string-index access, destructure). A bare
   word search was not enough — it passes on a comment or on a Strapi
   `fields: [...]` projection, and the projection in `listLessonsWithStatus`
   names `instrument`, so a word search would have called the fourth silent
   field rendered.

   Block fields are scoped to their own `case 'lesson.x':` region in
   LessonBody plus the leaf renderers, so dropping `caption` from ONE block
   fails even though five other cases still read a field by that name. Two
   allow-list entries, each with its reason in the file: `lesson.parameter.name`
   (one legal value, nothing to branch on) and `Lesson.order` (Strapi's sort
   key, never read client-side).

   What it cannot catch, stated in the file and worth repeating here: it is
   name-level, not type-level (`s.name` on a string set is indistinguishable
   from `parameter.name`); it cannot tell a used read from a dead one; and the
   inner shape of the `json` columns is invisible to it.
