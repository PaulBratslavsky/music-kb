...
function label(c: ProgressionChord): string {
  ...
  const suffix = QUALITY_LABELS[c.quality as ChordQuality] ?? c.quality;
...
export function ProgressionSheet({
  ...
  // ChordDiagram is 134×140 vertically; drawn horizontally (nut on the
  // left, as everywhere else in this app) those swap.
  const DW = guitar ? 140 : 112;
  const DH = guitar ? 134 : 64;
  const LABEL_H = 22;
  const PAD = 12;
