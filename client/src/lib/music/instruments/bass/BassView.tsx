...
export function BassView({
  ...
  return (
    ...
    >
      ...
      {/* Side-position dots above the neck — matches guitar convention */}
      {Array.from({ length: BASS_FRET_COUNT }, (_, i) => i + 1)
      ...
      {/* Strings — thicker for bass overall, with a small gradient by index. */}
      {grid.map((_, s) => (
        <line
          key={`string-${s}`}
      ...
      {/* Naturals overlay — same convention as GuitarView */}
      {showNaturals &&
        grid.flatMap((row) =>
          row
